...
export default function InjectedBuy1155(props: any) {
  ...
  async function handleBuy() {  
    try {
      ...
      const { marketplace } = getMarketplace() 
        const tx = await marketplace.buy1155NFTs(nonce)
          setTrigger(nonce)
          if(tx) {
          await tx.wait()
          setIsActive(true)
  ...
  return (
    <>
      <div className="buy1155config">
        <button 
          className="buy1155"
          onClick={handleBuy}
          disabled={isLoading}>Buy
        </button>
        {errorMessage && <p className="error1155buy">Error: {errorMessage}</p>}
      </div>
